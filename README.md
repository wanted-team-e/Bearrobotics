...
- 특정 기간 동안 특정 그룹에 속하는 POS 정보를 시간, 일, 주, 달, 연 중 하나로 그룹핑하여 POS 정보를 조회합니다.


**주소 API**

찾으려는 주소에 해당하는 POS 조회 : GET /api/address/주소이름/
- 찾으려는 주소가 있는지 조회하고 주소에 해당하는 레스토랑이 POS에 있는지 조회합니다.

주소 + 기간에 해당하는 POS정보 조회 : GET /api/address/주소이름/?start_date=&end_date=
- 찾으려는 주소가 있는지 조회하고 있을 경우 특정기간에 한해서 주소에 해당하는 레스토랑이 POS에 있는지 조회합니다.

주소 + 기간 + 시간에 해당하는 POS 조회 : GET /api/address/주소이름/?start_date=&end_date=&timeunit=
